...
// EOL matches an end of a line (a carriage return, line feed, or the end of
// state).
func EOL(state *State, result *Result) error {
	...
	if c != '\r' && c != '\n' {
		return NewError("expected carriage return, line feed or end of state", state.Position())
	}
	state.Advance()
	return nil
}
